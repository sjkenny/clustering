...
% for assigning average Z to each cluster
AverageZ_cluster=1;
...
for i=1:num_clusters
    ...
    sdx(i) = std(x_use);
    sdy(i) = std(y_use);
    if AverageZ_cluster==1
        z(idx_use) = out_avgZ(i);
    end
end
...
r.zc = z;
if AverageZ_cluster==1
    outfile = sprintf('%s-zCluster.bin',filehead)
    WriteMolBinNXcYcZc(r,outfile);
end
% 
% mol_idx_assign = find(ismember(frame,smallROI_idx_frame));

% r.cat(mol_idx_assign)=2;
